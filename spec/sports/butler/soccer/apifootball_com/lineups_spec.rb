...
RSpec.describe Sports::Butler::SoccerApi::ApifootballCom::Lineups do
  ...
  before do
    ...
    stubs_lineups_apifootball_com
  end

  describe 'when #by_match' do
    it_behaves_like 'when #by_match_array', 86392,
                    :response_lineups_all_apifootball_com, :parsed_response
  end
end

def stubs_lineups_apifootball_com
  stub_request(:get, "#{Sports::Butler::Configuration.api_endpoint[sport][api_name]}/?APIkey=my_dummy_token&action=get_lineups&id=86392")
    .to_return(status: 200, body: get_mocked_response('lineup.json', sport, api_name))
end

def response_lineups_all_apifootball_com
  {
    "86392": {
               "lineup": {
                           "home": {
