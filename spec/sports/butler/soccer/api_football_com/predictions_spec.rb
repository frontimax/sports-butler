...
RSpec.describe Sports::Butler::SoccerApi::ApiFootballCom::Predictions do
  ...
  before do
    ...
    stubs_predictions_api_football_com
  end

  describe 'when #by_match' do
    it_behaves_like 'when #by_match_array', 1540,
                    :response_predictions_one_api_football_com
  end
end

def stubs_predictions_api_football_com
  stub_request(:get, "#{Sports::Butler::Configuration.api_endpoint[sport][api_name]}/predictions?id=1540")
    .to_return(status: 200, body: get_mocked_response('predictions.json', sport, api_name))
end

def response_predictions_one_api_football_com
  [
    {
      "predictions": {
        "winner": {
