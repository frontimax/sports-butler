...
RSpec.describe Sports::Butler::SoccerApi::ApiFootballCom::Standings do
  ...
  before do
    ...
    stubs_standings_api_football_com
  end
  ...
  describe 'when #by_competition' do
    it_behaves_like 'when #by_competition', 78,
                    :response_standings_api_football_com
  end
end

def stubs_standings_api_football_com
  stub_request(:get, "#{Sports::Butler::Configuration.api_base_url[sport][api_name]}/standings?competition_id=78")
    .to_return(status: 200, body: get_mocked_response('standings.json', sport, api_name))
end

def response_standings_api_football_com
  [
    {
      "league": {
        "id": 78,
